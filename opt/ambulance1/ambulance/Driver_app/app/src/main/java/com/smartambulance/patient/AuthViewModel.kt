...
class AuthViewModel : ViewModel() {
    ...
    fun checkAuthStatus() {
        if (UserSession.isDriverLoggedIn) {
            authStatus = AuthStatus.Authenticated("driver", UserSession.isProfileComplete)
        } else if (UserSession.isPatientLoggedIn) {
    ...
    fun loginDriver(username: String, password: String) {
        if (username.isBlank() || password.isBlank()) {
            authStatus = AuthStatus.Error("Please enter both username and password.")
            return
        }
        ...
        viewModelScope.launch {
            try {
                val response = RetrofitClient.instance.loginDriver(com.smartambulance.patient.network.DriverLoginPayload(username, password))
                if (response.status == "success" && response.driver_id != null) {
                    UserSession.role = "driver"
                    UserSession.driverId = response.driver_id
                    UserSession.username = username
                    UserSession.isProfileComplete = true
                    authStatus = AuthStatus.Authenticated("driver", true)
                } else {
                    authStatus = AuthStatus.Error(response.message ?: "Invalid credentials.")
                }
            } catch (e: Exception) {
        ...
        viewModelScope.launch {
            try {
                ...
                if (response.status == "success") {
                    ...
                    // But for mobile, let's just log them in or show success.
                    // The user said "first open this login / register page... all let it be same"
                    // Let's just set the status to Idle so they can log in.
